...
import { ArrowLeft, Calendar, Check, ChevronRight, Coffee, CreditCard, Tv, Users, Wifi, Bath } from "lucide-react"
...
export default function RoomDetailsPage() {
  ...
  const { filters, user } = useStore((state) => ({
  ...
  const nights =
    filters.checkIn && filters.checkOut
      ? Math.ceil((filters.checkOut.getTime() - filters.checkIn.getTime()) / (1000 * 60 * 60 * 24))
      : 0
      ...
      <div className="container mx-auto py-10 px-4">
        <div className="flex items-center justify-center h-[60vh]">
          <div className="text-center">
            <h2 className="text-xl font-semibold mb-2">Loading room details...</h2>
    ...
    <div className="container mx-auto py-10 px-4">
      <div className="flex items-center mb-8">
        ...
        <div>
          ...
          <div className="flex items-center mt-1">
            ...
            {room.isResidential && <Badge className="ml-2 bg-primary">Residential</Badge>}
            {!room.isAvailable && (
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-8">
          {/* Image Gallery */}
          <ImageCarousel images={room.images} aspectRatio="video" className="rounded-lg overflow-hidden" />
          ...
          <Tabs defaultValue="overview">
            ...
            <TabsContent value="overview" className="space-y-4 pt-4">
              <div>
                ...
                <div className="mt-4 grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-muted-foreground">{room.isResidential ? "45-60" : "30-40"} m²</p>
                  </div>
                  <div>
              ...
              {room.isResidential && (
                <div className="mt-6">
                  <h2 className="text-xl font-semibold mb-2">Long-Term Stay Options</h2>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Card>
                      ...
                      <CardContent>
                        ...
                        <p className="text-sm text-muted-foreground">Flexible stay</p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-lg">Weekly</CardTitle>
                        <CardDescription className="text-xs text-green-600">Save 15%</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="text-2xl font-bold">${room.weeklyRate}</div>
                        <p className="text-sm text-muted-foreground">${Math.round(room.weeklyRate / 7)}/night</p>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                        <CardTitle className="text-lg">Monthly</CardTitle>
                        <CardDescription className="text-xs text-green-600">Save 30%</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="text-2xl font-bold">${room.monthlyRate}</div>
                        <p className="text-sm text-muted-foreground">${Math.round(room.monthlyRate / 30)}/night</p>
                      </CardContent>
                    </Card>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    ...
                    <li className="flex items-center">
                      <Check className="h-4 w-4 text-green-500" />
                      <span className="ml-2">Airport Transfer (on request)</span>
                    </li>
                  </ul>
                </div>
              </div>
            ...
            <TabsContent value="policies" className="space-y-4 pt-4">
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-muted-foreground mb-2">Check-in time starts at 3:00 PM</p>
                  <p className="text-muted-foreground">Check-out time is 12:00 PM</p>
                </div>

                <div>
                  <h3 className="text-lg font-semibold mb-2">Cancellation Policy</h3>
                  <p className="text-muted-foreground mb-2">Free cancellation up to 24 hours before check-in.</p>
                  <p className="text-muted-foreground">
                    Cancellations made less than 24 hours before check-in are subject to a charge equal to the first
                    night's stay.
                  </p>
                </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {roomTypes
                ...
                .slice(0, 2)
                .map((roomType) => (
                  <Card key={roomType.id} className="flex flex-col md:flex-row overflow-hidden">
                    ...
                    <div className="flex-1 p-4">
                      ...
                      <p className="text-sm text-muted-foreground line-clamp-2 mb-2">{roomType.description}</p>
                      <div className="flex items-center justify-between mt-auto">
                        <div className="text-lg font-semibold">
                          ${roomType.price}
                          <span className="text-sm font-normal text-muted-foreground">/night</span>
                        </div>
                        <Link href={`/rooms/${rooms.find((r) => r.type === roomType.id)?.id || 1}`}>
        ...
        <div>
          <Card className="sticky top-4">
            ...
            <CardContent className="space-y-4">
              ...
              {filters.checkIn && filters.checkOut ? (
                <div className="space-y-2 bg-muted p-3 rounded-md">
                  <div className="flex justify-between text-sm">
                    <span>Check-in:</span>
                    <span className="font-medium">{format(filters.checkIn, "EEE, MMM d, yyyy")}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Check-out:</span>
                    <span className="font-medium">{format(filters.checkOut, "EEE, MMM d, yyyy")}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <Link href="/rooms" className="text-xs text-primary hover:underline block text-right">
                    Change dates
                  </Link>
                </div>
              ) : (
              ...
              {/* Book Now Button */}
              <Button className="w-full" size="lg" disabled={!room.isAvailable || nights === 0} onClick={handleBookNow}>
...
function getFeatureIcon(feature: string) {
  if (feature.includes("WiFi")) return <Wifi className="h-4 w-4 text-muted-foreground" />
  if (feature.includes("Coffee")) return <Coffee className="h-4 w-4 text-muted-foreground" />
  if (feature.includes("TV")) return <Tv className="h-4 w-4 text-muted-foreground" />
